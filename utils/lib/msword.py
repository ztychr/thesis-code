...
from lib.ziplib import MODE_DIRECTORY, format_time_for_doc, zipinfo_contents_replace
...
def make_canary_msword(url: str, template: Path):
    ...
    with ZipFile(input_buf, "r") as doc:
        for entry in doc.filelist:
            ...
            contents = contents.replace("REPLACEME", url)
...
if __name__ == "__main__":  # pragma: no cover
    with open("testdoc.docx", "wb") as f:
        f.write(
            make_canary_msword(
                url="https://nordskov.net/honey?id=loool",
